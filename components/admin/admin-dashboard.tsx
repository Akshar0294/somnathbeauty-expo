...
type RegistrationRow = { id: string; registrationId: string; visitorName: string; phone: string; email: string | null; city: string; gender: string; paymentStatus: string; registrationStatus: string; amount: number; createdAt: string; eventName: string | null };
...
export function AdminDashboard({ email }: { email: string }) {
  ...
  const eventsQuery = useQuery({ queryKey: ["admin-events"], queryFn: () => adminJson<EventRecord[]>("/api/admin/events") });
  ...
  async function saveEvent() {
    ...
    setNotice("");
    try {
      await adminJson<EventRecord>("/api/admin/events", { method: editingId ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(editingId ? { ...eventDraft, id: editingId } : eventDraft) });
      await queryClient.invalidateQueries({ queryKey: ["admin-events"] });
      setNotice("Event saved.");
      if (!editingId) setEventDraft(blankEvent);
    } catch (error) {
  ...
  async function deleteEvent(id: string) {
    if (!window.confirm("Delete this event? Existing registrations may prevent deletion.")) return;
    try {
      await adminJson("/api/admin/events", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) });
      await queryClient.invalidateQueries({ queryKey: ["admin-events"] });
  ...
  function exportCsv() {
    const header = ["Registration ID", "Visitor", "Phone", "Email", "City", "Event", "Amount", "Payment", "Date"];
    const rows = registrations.map((row) => [row.registrationId, row.visitorName, row.phone, row.email ?? "", row.city, row.eventName ?? "", row.amount, row.paymentStatus, row.createdAt]);
  ...
  return <main className="admin-shell min-h-screen bg-[#f7f8fb]">
    ...
    <div className="mx-auto w-[min(calc(100%-30px),1180px)] py-8 md:w-[min(calc(100%-48px),1180px)]">
      <div className="mb-7 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-[.68rem] font-bold uppercase tracking-[.14em] text-rose">Soft Shine admin</p><h1 className="mt-2 font-display text-4xl font-normal tracking-[-.035em]">{nav.find(([key]) => key === tab)?.[1]}</h1><p className="mt-2 text-sm text-muted">Keep every public detail current from one place.</p></div><div className="flex gap-2">{notice && <span className="self-center text-xs text-success">{notice}</span>}{tab === "events" && <button type="button" className="inline-flex min-h-10 items-center rounded-full bg-ink px-4 text-xs font-bold text-white hover:bg-slate-700" onClick={newEvent}>Create event</button>}</div></div>
      ...
      {tab === "events" && <EventsPanel events={events} draft={eventDraft} setDraft={setEventDraft} editingId={editingId} onEdit={editEvent} onDelete={deleteEvent} onSave={saveEvent} onUpload={upload} busy={busy} />}
